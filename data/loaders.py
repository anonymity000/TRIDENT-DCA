...
class MiniImageNet(Dataset):
    ...
    * **target_transform** (Transform, *optional*, default=None) - Target pre-processing.
    ...
    def __init__(self,
        ...
        '''
        self.x = torch.from_numpy(np.array(self.data["data"])).permute(0, 3, 1, 2).float()       
        self.y = np.ones(len(self.x))
        self.y = torch.from_numpy(np.array(self.data['class_dict'], dtype=np.int8))
        '''
        ...
        '''
        a = torch.from_numpy(np.array(self.data["data"]))
        self.x = torch.from_numpy(np.array(self.data["idata"])).permute(0, 3, 1, 2).float()
        self.y = np.ones(len(self.x))

        self.class_idx = index_classes(self.data['label'].keys())
      
        
        for class_name, idxs in self.data['label'].items():
            for idx in idxs:
                self.y[idx] = self.class_idx[class_name]
...
class TieredImagenet(Dataset):
    ...
    * **download** (bool, *optional*, default=False) - Whether to download the dataset.
    ...
    def __init__(self, root, mode='train', transform=None, target_transform=None, download=False):
        ...
        images_path = os.path.join(tiered_imaganet_path, short_mode + '_images.pkl')
...
class FGVCAircraft(Dataset):
    ...
    * **download** (bool, *optional*, default=False) - Whether to download the dataset.
    ...
    def __init__(self, root, mode='all', transform=None, target_transform=None, download=False):
        ...
        with open('./data/aircraft_meta-info.json') as file:
