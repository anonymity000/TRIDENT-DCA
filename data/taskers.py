...
from data.loaders import CIFARFS, CUBirds200, MiniImageNet, Omniglotmix, TieredImagenet, VGGFlower102, FGVCAircraft, FGVCFungi
...
def gen_tasks(dataname, root, image_transforms=None, target_transforms=None, download=False, **task_transforms):
    ...
    if (dataname == 'omniglot'):
    ...
    elif (dataname == 'vggflower'):
        vggf = VGGFlower102(root, mode, transform=image_transforms,
                            target_transform=target_transforms, download=download)
        dataset = l2l.data.MetaDataset(vggf)
    ...
    elif (dataname == 'aircraft'):
        airc = FGVCAircraft(root, mode, transform=image_transforms,
                            target_transform=target_transforms, download=download)
        dataset = l2l.data.MetaDataset(airc)
        ...
        fungi = FGVCAircraft(root, mode, transform=image_transforms,
                            target_transform=target_transforms, download=download)
        dataset = l2l.data.MetaDataset(fungi)
