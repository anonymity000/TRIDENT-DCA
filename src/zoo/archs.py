...
class TADCEncoder(nn.Module):
    ...
    def forward(self, x, update: str):
        ...
        #FREN
        x_shot = x[:self.args.n_ways*self.args.k_shots]      
        x_query = x[self.args.n_ways*self.args.k_shots:]
        '''
        x_query_before = x_shot.flatten(start_dim=1)
        x_query_num = x_query_before.cpu().detach().numpy()
        np.savetxt("query_before.txt",x_query_num)
        '''
        x_s = x_shot.permute(2, 3, 0, 1)
        x_s = x_s.reshape(x_s.shape[0]*x_s.shape[1], x_s.shape[2], x_s.shape[3])
        x_s = self.slf_attn(x_s, x_s, x_s)   
        x_s = x_s.reshape(x_shot.shape[2], x_shot.shape[3], x_shot.shape[0], x_shot.shape[1])   
        x_s = x_s.permute(2, 3, 0, 1)
        
        
        x_q = x_query.permute(2, 3, 0, 1)
        x_q = x_q.reshape(x_q.shape[0]*x_q.shape[1], x_q.shape[2], x_q.shape[3])
        x_q = self.slf_attn(x_q, x_q, x_q)
        x_q = x_q.reshape(x_query.shape[2], x_query.shape[3], x_query.shape[0], x_query.shape[1])
        x_q = x_q.permute(2, 3, 0, 1)
        
        x_t = torch.cat([x_s, x_q], dim=0)
        ...
        #update = 'outer'
        if update == 'inner':
            x_t = x_t[:self.args.n_ways*self.args.k_shots] * G
        elif update == 'outer':
            x_t = x_t[self.args.n_ways*self.args.k_shots:] * G
        '''
        x_query_before = x_t.flatten(start_dim=1)
        x_query_num = x_query_before.cpu().detach().numpy()
