...
start = 0
#learner.load_state_dict(torch.load('./models/train-pre/model_last.pt', map_location=args.device))
...
for iter in tqdm.tqdm(range(start, args.iterations)):
    ...
    for batch in range(args.meta_batch_size):
        ...
        if evaluation_accuracy >= max_eval_accuracy:
            learner = learner.to('cpu')
            profiler.log_model(learner, opt, 'max_eval')
    ...
    if validation_accuracy >= max_val_accuracy:
            learner = learner.to('cpu')
            profiler.log_model(learner, opt, 'max_val')
            learner = learner.to(args.device)
